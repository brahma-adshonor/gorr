...
func (m *Main) Run(args ...string) error {
	...
	case "view":
		return newViewCommand(m).Run(args[1:]...)
	default:
...
The commands are:
    ...
    view        iterate all key/value for given bucket
...
// ViewCommand represents the "view" command execution.
...
func newViewCommand(m *Main) *ViewCommand {
	return &ViewCommand{
		Stdin:  m.Stdin,
		Stdout: m.Stdout,
		Stderr: m.Stderr,
...
// Run executes the command.
func (cmd *ViewCommand) Run(args ...string) error {
	// Parse flags.
	fs := flag.NewFlagSet("", flag.ContinueOnError)
	help := fs.Bool("h", false, "")
	if err := fs.Parse(args); err != nil {
	...
	if err != nil {
		return fmt.Errorf("open db failed, err:%s\n", err.Error())
	}
	...
	db.View(func(tx *bolt.Tx) error {
		...
		for k, v := c.First(); k != nil; k, v = c.Next() {
			fmt.Printf("key(%d-bytes)=%s, value(%d-bytes)=%s\n", len(k), k, len(v), v)
		}
		return nil
	})

	fmt.Printf("done dumping db data\n")
	return nil
}

func (cmd *ViewCommand) Usage() string {
	return strings.TrimLeft(`usage: bolt view PATH BUCKET`, "\n")
}
